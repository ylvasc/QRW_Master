...
class AllGatherOp(_ods_ir.OpView):
  ...
  def __init__(self, tensor, gatheringAxes, outSharding, *, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    ...
    attributes["gatheringAxes"] = (gatheringAxes if (
    isinstance(gatheringAxes, _ods_ir.Attribute) or
    not _ods_ir.AttrBuilder.contains('Sdy_ListOfAxisRefLists')) else
      _ods_ir.AttrBuilder.get('Sdy_ListOfAxisRefLists')(gatheringAxes, context=_ods_context))
    attributes["outSharding"] = (outSharding if (
    isinstance(outSharding, _ods_ir.Attribute) or
    not _ods_ir.AttrBuilder.contains('Sdy_TensorSharding')) else
      _ods_ir.AttrBuilder.get('Sdy_TensorSharding')(outSharding, context=_ods_context))
  ...
  @builtins.property
  def gatheringAxes(self):
    return self.operation.attributes["gatheringAxes"]

  @gatheringAxes.setter
  def gatheringAxes(self, value):
    if value is None:
      raise ValueError("'None' not allowed as value for mandatory attributes")
    self.operation.attributes["gatheringAxes"] = value

  @builtins.property
  def outSharding(self):
    return self.operation.attributes["outSharding"]

  @outSharding.setter
  def outSharding(self, value):
    if value is None:
      raise ValueError("'None' not allowed as value for mandatory attributes")
    self.operation.attributes["outSharding"] = value
...
def all_gather(tensor, gathering_axes, out_sharding, *, loc=None, ip=None) -> _ods_ir.Value:
  return AllGatherOp(tensor=tensor, gatheringAxes=gathering_axes, outSharding=out_sharding, loc=loc, ip=ip).result
